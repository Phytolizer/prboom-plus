		...
		for (i = 0, h = 0; i < sec->linecount; i++) {
			...
			if (other->floorheight > height) {
				...
				if (compatibility_level < dosdoom_compatibility && h >= MAX_ADJOINING_SECTORS) {
					lprintf(LO_WARN,
							"P_FindNextHighestFloor: Overflow of heightlist[%d] array is "
					        "detected.\n",
	...
	switch ((line->special & LockedKey) >> LockedKeyShift) {
		...
		case BCard:
			if (!player->cards[it_bluecard] && (!skulliscard || !player->cards[it_blueskull])) {
				player->message =
						skulliscard ? s_PD_BLUEK : s_PD_BLUEC; // Ty 03/27/98 - externalized
				S_StartSound(player->mo, sfx_oof);             // killough 3/20/98
				return false;
			}
			break;
		...
		case BSkull:
			if (!player->cards[it_blueskull] && (!skulliscard || !player->cards[it_bluecard])) {
				player->message =
						skulliscard ? s_PD_BLUEK : s_PD_BLUES; // Ty 03/27/98 - externalized
				S_StartSound(player->mo, sfx_oof);             // killough 3/20/98
				return false;
			}
			break;
		...
		case AllKeys:
			...
			if (skulliscard &&
					((!player->cards[it_redcard] && !player->cards[it_redskull]) ||
							(!player->cards[it_bluecard] && !player->cards[it_blueskull]) ||
							// e6y
			                // Compatibility with buggy MBF behavior when 3-key door works with only
			                // 2 keys There is no more desync on 10sector.wad\ts27-137.lmp
	...
	if (!demo_compatibility) // generalized types not recognized if old demo
	{
		...
		if ((unsigned)line->special >= GenEnd) {
			...
			if (((line->special & TriggerType) == WalkOnce) ||
					((line->special & TriggerType) ==
							WalkMany)) { // jff 4/1/98 check for being a walk type before reporting
				                         // door type
	...
	if (!demo_compatibility) {
		...
		if ((unsigned)line->special >= GenEnd) {
			...
			if (((line->special & TriggerType) == GunOnce) ||
					((line->special & TriggerType) ==
							GunMany)) { // jff 4/1/98 check for being a gun type before reporting
				                        // door type
				if (!P_CanUnlockGenDoor(line, thing->player))
					return;
			} else
...
void T_Scroll(scroll_t* s) {
	...
	if (s->control != -1) { // compute scroll amounts based on a sector's height changes
...
static void P_SpawnFriction(void) {
	...
	for (i = 0; i < numlines; i++, l++)
		if (l->special == 223) {
			...
			if (mbf_features) { // killough 8/28/98: prevent odd situations
...
static dboolean PIT_PushThing(mobj_t* thing) {
	...
	if (!mbf_features ? thing->player && !(thing->flags & (MF_NOCLIP | MF_NOGRAVITY))
