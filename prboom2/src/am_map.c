	...
	for (i = 0; i < numlines; i++) {
		...
		if (ddt_cheating || (lines[i].flags & ML_MAPPED)) {
			...
			if /* jff 4/23/98 add exit lines to automap */
					(mapcolor_exit &&
							(lines[i].special == 11 || lines[i].special == 52 ||
									lines[i].special == 197 || lines[i].special == 51 ||
									lines[i].special == 124 || lines[i].special == 198)) {
			...
			} else                                   /* now for 2S lines */
			{
				...
				if (mapcolor_tele && !(lines[i].flags & ML_SECRET) &&
						(lines[i].special == 39 || lines[i].special == 97 ||
								lines[i].special == 125 ||
								lines[i].special == 126)) { // teleporters
...
static void AM_drawPlayers(void) {
	...
	if (!netgame) {
		...
		if (ddt_cheating)
			AM_drawLineCharacter(cheat_player_arrow, NUMCHEATPLYRLINES, scale, viewangle,
...
static void AM_ProcessNiceThing(mobj_t* mobj, angle_t angle, fixed_t x, fixed_t y) {
	...
	static const map_nice_icon_param_t icons[] = {{SPR_STIM, am_icon_health, 12, 0, 100, 100, 200},
			...
			{SPR_YSKU, am_icon_key, 10, 0, 255, 255, 0}, {SPR_RKEY, am_icon_key, 10, 0, 255, 0, 0},
	...
	if (fx + shadow_radius < 0 || fx - shadow_radius > (float)SCREENWIDTH ||
			fy + shadow_radius < 0 || fy - shadow_radius > (float)SCREENHEIGHT) {
		return;
	}
	...
	for (i = 0; i < numsectors; i++) {
		...
		for (pass = 0; pass < 2; pass += (enemies ? 1 : 2)) {
			...
			while (t) // for all things in that sector
			{
				...
				AM_drawLineCharacter(thintriangle_guy, NUMTHINTRIANGLEGUYLINES, scale, angle,
