...
static void L_CommandOnChange(void) {
	...
	switch (index) {
		case 0:
			...
			MessageBox(launcher.HWNDServer, "The cache has been successfully rebuilt",
		...
		case 4: {
			...
			if (cmdline) {
				...
				if (FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL,
							result, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (char*)&msg, 512,
							NULL)) {
		...
		case 5: {
			...
			if (MessageBox(launcher.HWNDServer, buf, LAUNCHER_CAPTION,
				...
				strcpy(buf,
						((launcher_enable + 1) % launcher_enable_count == launcher_enable_never
										? "Disable"
										: "Enable"));
						...
						if (launcher.files[i].source == source_iwad &&
							...
							for (j = 0; !processed && j < listIWADCount; j++) {
								if (SendMessage(launcher.listIWAD, CB_GETITEMDATA, j, 0) == i) {
									if (SendMessage(launcher.listIWAD, CB_SETCURSEL, j, 0) !=
											CB_ERR) {
										processed = true;
										L_GameOnChange();
									}
				...
				*p = 0;
				if (3 == sscanf(p + 1, "%d, %d, %d", &item.source, &item.doom1, &item.doom2)) {
					launcher.cache = realloc(
							launcher.cache, sizeof(*launcher.cache) * (launcher.cachesize + 1));
...
static void L_SelAdd(int index) {
	launcher.selection = realloc(
			launcher.selection, sizeof(launcher.selection[0]) * (launcher.selectioncount + 1));
...
static void L_FillFilesList(fileitem_t* iwad) {
	...
	for (i = 0; i < launcher.filescount; i++) {
		...
		if (iwad->doom1 && item->doom1 || iwad->doom2 && item->doom2 ||
			...
			index = (int)SendMessage(launcher.listPWAD, LB_ADDSTRING, 0,
	...
	switch (message) {
		case WM_INITDIALOG: {
			...
			if (hMod) {
				...
				pEnableThemeDialogTexture = (EnableThemeDialogTexturePROC)GetProcAddress(
			...
			SendMessage(
					launcher.listCMD, CB_ADDSTRING, 0, (LPARAM) "... with DOOM demos (-auto mode)");
		...
		switch (idCtrl) {
			...
			case IDC_PWADLIST: {
				...
				for (i = 0; i < selectioncount; i++) {
					...
					int needlen = (tooltip_str ? strlen(tooltip_str) : 0) + strlen(line) +
							sizeof(char) * 8;
