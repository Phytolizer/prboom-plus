...
void gld_CheckHardwareGamma(void) {
	gl_DeviceSupportsGamma = (-1 !=
			SDL_GetWindowGammaRamp(sdl_window, gl_oldHardwareGamma[0], gl_oldHardwareGamma[1],
	...
	if (gl_DeviceSupportsGamma) {
		...
		// if ((HIBYTE(gl_oldHardwareGamma[0][247]) == 255))
		{
			...
			lprintf(LO_WARN,
					"gld_CheckHardwareGamma: suspicious gamma tables, using linear ramp for "
			        "restoration\n");
...
int gld_SetGammaRamp(int gamma) {
	...
	if (gamma == -1) {
		succeeded = (SDL_SetWindowGammaRamp(sdl_window, gl_oldHardwareGamma[0],
							 gl_oldHardwareGamma[1], gl_oldHardwareGamma[2]) != -1);
	} else {
