...
floater:
	...
	if (mo->z <= mo->floorz) {
		...
		if (mo->momz < 0) {
			...
			if (mo->flags & MF_TOUCHY && mo->intflags & MIF_ARMED && mo->health > 0)
				...
				if (mo->player && /* killough 5/12/98: exclude voodoo dolls */
				        ...
				        // because original sources do not exclude voodoo dolls from condition
				        // above, but Boom does it.
		...
		if (mo->type == MT_GIBDTH && !demorecording &&
				!demoplayback) { // if (mo->flags & MF_LOGRAV)
...
void P_MobjThinker(mobj_t* mobj) {
	...
	if (mobj->z != mobj->floorz || mobj->momz) {
		...
		if (mobj->thinker.function != P_MobjThinker)            // cph - Must've been removed
			return;                                             // killough - mobj was removed
	} else if (!(mobj->momx | mobj->momy) && !sentient(mobj)) { // non-sentient objects at rest
...
mobj_t* P_SpawnMapThing(const mapthing_t* mthing, int index) {
	...
	if (thingtype <= 4 && thingtype > 0) // killough 2/26/98 -- fix crashes
	{
		...
		if (!netgame && thingtype > 1 && thingtype <= dogs + 1 &&
				!players[thingtype - 1].secretcount) { // use secretcount to avoid multiple dogs in
			                                           // case of multiple starts
...
mobj_t* P_SpawnMissile(mobj_t* source, mobj_t* dest, mobjtype_t type) {
	...
	if (dest->flags & MF_SHADOW) { // killough 5/5/98: remove dependence on order of evaluation:
