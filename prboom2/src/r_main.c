...
angle_t R_PointToAngleSlope(fixed_t x1, fixed_t y1, fixed_t x, fixed_t y, slope_div_fn slope_div) {
	return (y -= y1, (x -= x1) || y) ? x >= 0 ? y >= 0 ? (x > y) ? tantoangle[slope_div(y, x)]
																 : // octant 0
...
static void R_InitTextureMapping(void) {
	...
	if (wide_centerx !=
			centerx) { // wide_centerx is what centerx would be if the display was not widescreen
...
static void InitStretchParam(
	...
	if (flags == VPT_ALIGN_RIGHT || flags == VPT_ALIGN_RIGHT_BOTTOM ||
			flags == VPT_ALIGN_RIGHT_TOP) {
		offsets->deltax1 *= 2;
		offsets->deltax2 *= 2;
	}
...
void R_RenderPlayerView(player_t* player) {
	...
	if (V_GetMode() == VID_MODEGL) {
		...
		if (flashing_hom) { // killough 2/10/98: add flashing red HOM indicators
