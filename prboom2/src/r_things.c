...
static void R_DrawPSprite(pspdef_t* psp) {
	...
	if ((psp->state->frame & FF_FRAMEMASK) >= sprdef->numframes)
		I_Error("R_ProjectSprite: Invalid sprite frame %i : %li", psp->state->sprite,
				psp->state->frame);
#endif
...
#define bcopyp(d, s, n) \
	asm(" cld; rep; movsl;" ::"D"(d), "S"(s), "c"(n) : "%cc", "%esi", "%edi", "%ecx", "memory")
...
static void R_DrawSprite(vissprite_t* spr) {
	...
	if (spr->heightsec != -1) // only things in specially marked sectors
	{
		...
		if ((mh = sectors[spr->heightsec].floorheight) > spr->gz &&
			...
			if (mh <= 0 || (phs != -1 && viewz > sectors[phs].floorheight)) { // clip bottom
				for (x = spr->x1; x <= spr->x2; x++)
		...
		if ((mh = sectors[spr->heightsec].ceilingheight) < spr->gzt &&
				(h = centeryfrac - FixedMul(mh - viewz, spr->scale)) >= 0 &&
				(h >>= FRACBITS) < viewheight) {
			if (phs != -1 && viewz >= sectors[phs].ceilingheight) { // clip bottom
				for (x = spr->x1; x <= spr->x2; x++)
					if (clipbot[x] == -2 || h < clipbot[x])
						clipbot[x] = h;
			} else // clip top
