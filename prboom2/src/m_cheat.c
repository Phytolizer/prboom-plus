...
static void cheat_god() { // 'dqd' cheat for toggleable god mode
	...
	if (plyr->playerstate == PST_DEAD) {
		...
		P_SpawnMobj(plyr->mo->x + 20 * finecosine[an], plyr->mo->y + 20 * finesine[an], plyr->mo->z,
				MT_TFOG);
...
static void cheat_comp() {
	...
	G_Compatibility();
	doom_printf("New compatibility level:\n%s (%d)", comp_lev_str[compatibility_level],
			compatibility_level);
}
		...
		while ((currentthinker = P_NextThinker(currentthinker, th_all)) != NULL)
			if (currentthinker->function == P_MobjThinker &&
					...
					(((mobj_t*)currentthinker)->flags & MF_COUNTKILL ||
							((mobj_t*)currentthinker)->type ==
									MT_SKULL)) { // killough 3/6/98: kill even if PE is dead
...
static void cheat_tntweap() {                // Ty 03/27/98 - *not* externalized
...
static void cheat_tntammox(buf) char buf[1];
{
	...
	{                               // killough 5/5/98: switch plasma and rockets for now -- KLUDGE
		...
		plyr->message = (plyr->ammo[a] = !plyr->ammo[a]) ? plyr->ammo[a] = plyr->maxammo[a],
