...
void G_BuildTiccmd(ticcmd_t* cmd) {
	...
	if ((!demo_compatibility && players[consoleplayer].attackdown && // killough
		...
		if (next_weapon) {
			...
			newweapon = (gamekeydown[key_weapon1] || mousebuttons[mb_weapon1])
					? wp_fist
					...
					: (gamekeydown[key_weapon6] || mousebuttons[mb_weapon6]) &&
							gamemode != shareware
					? wp_plasma
					: (gamekeydown[key_weapon7] || mousebuttons[mb_weapon7]) &&
							gamemode != shareware
					? wp_bfg
					: (gamekeydown[key_weapon8] || mousebuttons[mb_weapon8]) ? wp_chainsaw
					: (!demo_compatibility &&
							  (gamekeydown[key_weapon9] || mousebuttons[mb_weapon9]) &&
							  gamemode == commercial)
					? wp_supershotgun
					: wp_nochange;
		}
	...
	if (strafe) {
		side += mousex /
				movement_mousestrafedivisor; /* mead  Don't want to strafe as fast as turns.*/
...
void G_DeathMatchSpawnPlayer(int playernum) {
	...
	if (selections < MAXPLAYERS)
		I_Error("G_DeathMatchSpawnPlayer: Only %i deathmatch spots, %d required", selections,
				MAXPLAYERS);
...
const char* comp_lev_str[MAX_COMPATIBILITY_LEVEL] = {"Doom v1.2", "Doom v1.666", "Doom/Doom2 v1.9",
		...
		"boom v2.01", "boom v2.02", "lxdoom v1.3.2+", "MBF", "PrBoom 2.03beta",
		"PrBoom v2.1.0-2.1.1", "PrBoom v2.1.2-v2.2.6", "PrBoom v2.3.x", "PrBoom 2.4.0",
		"Current PrBoom"};
...
static void G_DoSaveGame(dboolean menu) {
	...
	*save_p++ = 0xe6; // consistancy marker
	...
	Z_CheckHeap();
	doom_printf("%s",
			M_WriteFile(name, savebuffer, save_p - savebuffer)
...
void G_Compatibility(void) {
	static const struct {
		...
		complevel_t opt; // level at which fix/change was made optional
			...
			// comp_telefrag - monsters used to telefrag only on MAP30, now they do it for spawners
	        // only
...
void G_RecordDemo(const char* name) {
	...
	demofp = NULL;
	if (access(demoname, F_OK) || democontinue || (demo_compatibility && demo_overwriteexisting)) {
...
static dboolean CheckForOverrun(const byte* start_p, const byte* current_p, size_t maxsize,
		size_t size, dboolean failonerror) {
...
void G_SaveRestoreGameOptions(int save) {
	...
	static gameoption_t gameoptions[] = {{1, 0, &demover}, {1, 0, (int*)&compatibility_level},
			...
			{1, 0, &demo_insurance}, {1, 0, &monster_infighting}, {1, 0, &dogs},
			{1, 0, &distfriend}, {1, 0, &monster_backing}, {1, 0, &monster_avoid_hazards},
			{1, 0, &monster_friction}, {1, 0, &help_friends}, {1, 0, &dog_jumping},
			{1, 0, &monkeys},
...
const byte* G_ReadDemoHeaderEx(const byte* demo_p, size_t size, unsigned int params) {
	...
	if (demover == 255) {
		...
		if (strncmp((const char*)demo_p, prdemosig, 5) != 0) {
			I_Error("G_ReadDemoHeader: Extended demo format 255 found, but \"PR+UM\" string not "
			        "found.");
		}
		...
		if (extension_version != 1) {
			I_Error("G_ReadDemoHeader: Extended demo format version %d unrecognized.",
					extension_version);
		}
	...
	} else // new versions of demos
	{
		...
		switch (demover) {
			...
			case 203:
				...
				switch (*(header_p + 2)) {
					case 'B': /* LxDoom */
						/* cph - DEMOSYNC - LxDoom demos recorded in compatibility modes support
						 * dropped */
...
void P_WalkTicker() {
	...
	walkcamera.x += FixedMul(
			(ORIG_FRICTION / 4) * forward, finecosine[walkcamera.angle >> ANGLETOFINESHIFT]);
	walkcamera.y +=
