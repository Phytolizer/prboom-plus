...
static dboolean P_CheckMissileRange(mobj_t* actor) {
	...
	if (actor->flags & MF_JUSTHIT) { // the target just hit the enemy, so fight back!
				...
				P_AvoidDropoff(actor)) /* Move away from dropoff */
		{
			...
			if (actor->flags & target->flags & MF_FRIEND && distfriend << FRACBITS > dist &&
				...
				deltax = -deltax, deltay = -deltay;
					...
					(actor->flags ^ target->flags) & MF_FRIEND) { // Live enemy target
								...
								(target->player && dist < MELEERANGE * 3 &&
										(target->player->readyweapon == wp_fist ||
												target->player->readyweapon ==
														wp_chainsaw)))) { // Back away from melee
					                                                      // attacker
...
static dboolean P_LookForPlayers(mobj_t* actor, dboolean allaround) {
	...
	if (actor->flags &
			MF_FRIEND) { // killough 9/9/98: friendly monsters go about players differently
		int anyone;
	...
	for (th = cap->cnext; th != cap; th = th->cnext)
		if (((mobj_t*)th)->health * 2 >= ((mobj_t*)th)->info->spawnhealth) {
			if (P_Random(pr_helpfriend) < 180)
				...
				((mobj_t*)th)->target != actor->target && !PIT_FindTarget(((mobj_t*)th)->target)) {
...
void A_FaceTarget(mobj_t* actor) {
	...
	if (actor->target->flags &
			MF_SHADOW) { // killough 5/5/98: remove dependence on order of evaluation:
...
void A_SPosAttack(mobj_t* actor) {
	...
	for (i = 0; i < 3; i++) { // killough 5/5/98: remove dependence on order of evaluation:
...
void P_SpawnBrainTargets(void) // killough 3/26/98: renamed old function
{
	...
	for (thinker = thinkercap.next; thinker != &thinkercap; thinker = thinker->next)
		if (thinker->function == P_MobjThinker) {
			...
			if (m->type == MT_BOSSTARGET) { // killough 2/7/98: remove limit on icon landings:
...
void A_BrainExplode(mobj_t* mo) { // killough 5/5/98: remove dependence on order of evaluation:
