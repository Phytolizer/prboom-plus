...
#define ntohs(x) \
	(/*(short int)*/ ( \
			(((unsigned short int)(x)&0x00ff) << 8) | (((unsigned short int)(x)&0xff00) >> 8)))
...
static dboolean ReadVariableLength(unsigned int* result, midimem_t* mf) {
	...
	*result = 0;
	...
	for (i = 0; i < 4; ++i) {
		if (!ReadByte(&b, mf)) {
			lprintf(LO_WARN,
					"ReadVariableLength: Error while reading "
	...
	lprintf(LO_WARN,
			"ReadVariableLength: Variable-length value too "
...
static dboolean ReadChannelEvent(
	...
	if (!ReadByte(&b, mf)) {
		lprintf(LO_WARN,
				"ReadChannelEvent: Error while reading channel "
				"event parameters\n");
		return false;
	}
...
static dboolean ReadSysExEvent(midi_event_t* event, int event_type, midimem_t* mf) {
	...
	if (!ReadVariableLength(&event->data.sysex.length, mf)) {
		lprintf(LO_WARN,
				"ReadSysExEvent: Failed to read length of "
				"SysEx block\n");
		return false;
	}
...
static dboolean ReadMetaEvent(midi_event_t* event, midimem_t* mf) {
	...
	if (!ReadVariableLength(&event->data.meta.length, mf)) {
		lprintf(LO_WARN,
				"ReadMetaEvent: Failed to read length of "
				"MetaEvent block\n");
		return false;
	}
...
static dboolean ReadTrack(midi_track_t* track, midimem_t* mf) {
	...
	for (;;) {
		...
		*/
		if (track->num_events == track->num_event_mem) { // depending on the state of the heap and
			                                             // the malloc implementation, realloc()
			// one more event at a time can be VERY slow.  10sec+ in MSVC
			track->num_event_mem += 100;
			new_events = (midi_event_t*)realloc(
		...
		*epos = file->tracks[nextrk].events + trackpos[nextrk];
		...
		if (epos[0]->event_type == MIDI_EVENT_META &&
				epos[0]->data.meta.type ==
						MIDI_META_END_OF_TRACK) { // change end of track into no op
			trackactive--;
			trackpos[nextrk] = -1;
...
static double compute_spmc_normal(
		unsigned mpq, unsigned tempo, unsigned sndrate) { // returns samples per midi clock
...
static double compute_spmc_smpte(
		unsigned smpte_fps, unsigned mpf, unsigned sndrate) { // returns samples per midi clock
		...
		if (ev->event_type == MIDI_EVENT_META) {
			if (ev->data.meta.length == 3) {
				tempo = (unsigned)ev->data.meta.data[0] << 16 |
						(unsigned)ev->data.meta.data[1] << 8 | (unsigned)ev->data.meta.data[2];
			} else
