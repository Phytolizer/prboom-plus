...
void alsaplay_refresh_outputs(void) {
	...
	if (!seq_handle) {
		lprintf(LO_WARN,
				"alsaplay_refresh_outputs: Can't list ALSA output ports: seq_handle is not "
		        "initialized\n");
		return;
	}
...
int alsaplay_connect_output(int which) {
	if (which >= alsaplayer_num_outs) {
		lprintf(LO_WARN,
				"alsaplay_connect_output: tried to connect to output listing at index out of "
		        "bounds: %d\n",
				which);
		return -1;
	}
...
int alsa_midi_default_dest(void) {
	...
	if (!seq_handle) {
		lprintf(LO_WARN,
				"alsa_midi_default_dest: Can't list ALSA output ports: seq_handle is not "
		        "initialized\n");
		return 0;
	}
		...
		while (snd_seq_query_next_port(seq_handle, pinfo) == 0) {
			...
			if (loopback_cl == client_num) {
				...
				if (alsa_midi_set_dest(client_num, port_num) != 0) {
					lprintf(LO_WARN,
							"alsa_midi_default_dest: error connecting to default port %i:%i (%s): "
					        "%s\n",
	...
	if (loopback_cl != -1) {
		if ((status = alsa_midi_set_dest(loopback_cl, loopback_prt)) != 0) {
			lprintf(LO_WARN,
					"alsa_midi_default_dest: (fallback) error connecting to default port %i:%i "
			        "(%s): %s\n",
...
static unsigned long alsa_now(void) {
	...
	if (time == 0) {
		lprintf(LO_WARN,
				"alsaplayer: alsa_now(): error getting realtime position from queue status\n");
		return 0;
	}
...
static int alsa_midi_init_connect_default_port(void) {
	...
	if (snd_mididev && strlen(snd_mididev)) {
		...
		CHK_LPRINT_ERR(snd_seq_parse_address(seq_handle, &seqaddr, snd_mididev), LO_WARN,
				"alsa_init: Error connecting to configured MIDI output port \"%s\": %s",
				snd_mididev)
...
static void alsa_render(void* vdest, unsigned bufflen) {
	...
	while (1) {
		...
		if (samples + sampleswritten >
				length) { // overshoot; render some samples without processing an event
			break;
		}
	...
	if (samples + sampleswritten >
			length) { // broke due to next event being past the end of current render buffer
