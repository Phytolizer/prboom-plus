...
static deh_strs deh_strlookup[] = {
		...
		{&s_DETAILHI, "DETAILHI"}, {&s_DETAILLO, "DETAILLO"}, {&s_GAMMALVL0, "GAMMALVL0"},
		{&s_GAMMALVL1, "GAMMALVL1"}, {&s_GAMMALVL2, "GAMMALVL2"}, {&s_GAMMALVL3, "GAMMALVL3"},
		{&s_GAMMALVL4, "GAMMALVL4"}, {&s_EMPTYSTRING, "EMPTYSTRING"}, {&s_GOTARMOR, "GOTARMOR"},
		{&s_GOTMEGA, "GOTMEGA"}, {&s_GOTHTHBONUS, "GOTHTHBONUS"}, {&s_GOTARMBONUS, "GOTARMBONUS"},
		{&s_GOTSTIM, "GOTSTIM"}, {&s_GOTMEDINEED, "GOTMEDINEED"}, {&s_GOTMEDIKIT, "GOTMEDIKIT"},
		...
		{&s_GOTSHELLS, "GOTSHELLS"}, {&s_GOTSHELLBOX, "GOTSHELLBOX"},
		{&s_GOTBACKPACK, "GOTBACKPACK"}, {&s_GOTBFG9000, "GOTBFG9000"},
		{&s_GOTCHAINGUN, "GOTCHAINGUN"}, {&s_GOTCHAINSAW, "GOTCHAINSAW"},
		...
		{&s_STSTR_CHOPPERS, "STSTR_CHOPPERS"}, {&s_STSTR_CLEV, "STSTR_CLEV"},
		{&s_STSTR_COMPON, "STSTR_COMPON"}, {&s_STSTR_COMPOFF, "STSTR_COMPOFF"},
		{&s_E1TEXT, "E1TEXT"}, {&s_E2TEXT, "E2TEXT"}, {&s_E3TEXT, "E3TEXT"}, {&s_E4TEXT, "E4TEXT"},
		{&s_C1TEXT, "C1TEXT"}, {&s_C2TEXT, "C2TEXT"}, {&s_C3TEXT, "C3TEXT"}, {&s_C4TEXT, "C4TEXT"},
		{&s_C5TEXT, "C5TEXT"}, {&s_C6TEXT, "C6TEXT"}, {&s_P1TEXT, "P1TEXT"}, {&s_P2TEXT, "P2TEXT"},
		{&s_P3TEXT, "P3TEXT"}, {&s_P4TEXT, "P4TEXT"}, {&s_P5TEXT, "P5TEXT"}, {&s_P6TEXT, "P6TEXT"},
		{&s_T1TEXT, "T1TEXT"}, {&s_T2TEXT, "T2TEXT"}, {&s_T3TEXT, "T3TEXT"}, {&s_T4TEXT, "T4TEXT"},
		{&s_T5TEXT, "T5TEXT"}, {&s_T6TEXT, "T6TEXT"}, {&s_CC_ZOMBIE, "CC_ZOMBIE"},
		...
		{&s_CC_HERO, "CC_HERO"}, {&bgflatE1, "BGFLATE1"}, {&bgflatE2, "BGFLATE2"},
		{&bgflatE3, "BGFLATE3"}, {&bgflatE4, "BGFLATE4"}, {&bgflat06, "BGFLAT06"},
		{&bgflat11, "BGFLAT11"}, {&bgflat20, "BGFLAT20"}, {&bgflat30, "BGFLAT30"},
		{&bgflat15, "BGFLAT15"}, {&bgflat31, "BGFLAT31"}, {&bgcastcall, "BGCASTCALL"},
		...
		{&startup4, "STARTUP4"}, {&startup5, "STARTUP5"},
...
static const deh_bexptr deh_bexptrs[] = // CPhipps - static const
		{
				...
				{A_GunFlash, "A_GunFlash"}, {A_FireMissile, "A_FireMissile"}, {A_Saw, "A_Saw"},
				{A_FirePlasma, "A_FirePlasma"}, {A_BFGsound, "A_BFGsound"},
				{A_FireBFG, "A_FireBFG"}, {A_BFGSpray, "A_BFGSpray"}, {A_Explode, "A_Explode"},
				{A_Pain, "A_Pain"}, {A_PlayerScream, "A_PlayerScream"}, {A_Fall, "A_Fall"},
				...
				{A_StartFire, "A_StartFire"}, {A_Fire, "A_Fire"}, {A_FireCrackle, "A_FireCrackle"},
				{A_Tracer, "A_Tracer"}, {A_SkelWhoosh, "A_SkelWhoosh"}, {A_SkelFist, "A_SkelFist"},
				...
				{A_BrainScream, "A_BrainScream"}, {A_BrainDie, "A_BrainDie"},
				{A_BrainAwake, "A_BrainAwake"}, {A_BrainSpit, "A_BrainSpit"},
				{A_SpawnSound, "A_SpawnSound"}, {A_SpawnFly, "A_SpawnFly"},
...
void ProcessDehFile(const char* filename, const char* outfilename, int lumpnum) {
	...
	while (dehfgets(inbuffer, sizeof(inbuffer), filein)) {
		...
		for (match = 0, i = 0; i < DEH_BLOCKMAX; i++)
			if (!strncasecmp(
						inbuffer, deh_blocks[i].key, strlen(deh_blocks[i].key))) { // matches one
				if (i < DEH_BLOCKMAX - 1)
					match = 1;
...
static void deh_procBexCodePointers(DEHFILE* fpin, FILE* fpout, char* line) {
	...
	while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' ')) {
		...
		{
			++i;
			if (!stricmp(key, deh_bexptrs[i].lookup)) { // Ty 06/01/98  - add  to states[].action
				                                        // for new djgcc version
...
static uint_64_t getConvertedDEHBits(uint_64_t bits) {
	static const uint_64_t bitMap[32] = {
			...
			MF_SOLID,      // 1 Obstacle - The thing is solid and will not let you (or others) pass
	                       // through it
			...
			MF_DROPOFF, // 10 Travels over cliffs - Monsters normally do not walk off ledges/steps
	                    // they could not walk up. With this set they can walk off any height of
	                    // cliff. Usually only used for flying monsters.
			MF_PICKUP,  // 11 Pick up items - The thing can pick up gettable items.
			MF_NOCLIP,  // 12 No clipping - Thing can walk through walls.
			MF_SLIDE,   // 13 Slides along walls - Keep info about sliding along walls (don't really
	                    // know much about this one).
			MF_FLOAT,   // 14 Floating - Thing can move to any height
			MF_TELEPORT, // 15 Semi no clipping - Don't cross lines or look at teleport heights.
	                     // (don't really know much about this one either).
			MF_MISSILE,  // 16 Projectiles - Behaves like a projectile, explodes when hitting
	                     // something that blocks movement
			MF_DROPPED,  // 17 Disappearing weapon - Dropped, not spawned (like an ammo clip) I have
	                     // not had much success in using this one.
			MF_SHADOW,   // 18 Partial invisibility - Drawn like a spectre.
			MF_NOBLOOD,  // 19 Puffs (vs. bleeds) - If hit will spawn bullet puffs instead of blood
	                     // splats.
			...
			MF_COUNTKILL, // 22 Affects kill % - counted as a killable enemy and affects percentage
	                      // kills on level summary.
						...
						for (iy = 0; iy < DEH_MOBJFLAGMAX; iy++) {
							...
							if (fpout) {
								fprintf(fpout, "ORed value 0x%08lX%08lX %s\n",
										...
										(unsigned long)deh_mobjflags[iy].value & 0xffffffff,
										strval);
							}
...
static void deh_procPointer(DEHFILE* fpin, FILE* fpout, char* line) // done
{
	...
	while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' ')) {
		...
		if (!deh_strcasecmp(key, deh_state[4])) // Codep frame (not set in Frame deh block)
		{
			...
			if (fpout)
				fprintf(fpout, " - applied from codeptr[%ld] to states[%d]\n", (long)value,
						indexnum);
...
static void deh_procPars(DEHFILE* fpin, FILE* fpout, char* line) // extension
{
	...
	while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' ')) {
		...
		if (!*inbuffer)
			break;                                                               // killough 11/98
		if (3 != sscanf(inbuffer, "par %i %i %i", &episode, &level, &partime)) { // not 3
			if (2 != sscanf(inbuffer, "par %i %i", &level, &partime)) {          // not 2
				if (fpout)
	...
	*holdstring = '\0'; // empty string to start with
	...
	while (!dehfeof(fpin) && *inbuffer) /* && (*inbuffer != ' ') */
	{
		...
		if (*holdstring) // didn't have a backslash, trap above would catch that
		{
			...
			found = deh_procStringSub(
					key, NULL, holdstring, fpout); // supply keyand not search string
