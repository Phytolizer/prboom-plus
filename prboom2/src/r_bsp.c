		...
		if (underwater &&
			...
			if (ceilinglightlevel)
				*ceilinglightlevel = s->ceilinglightsec == -1
				...
				sec->ceilingheight > s->ceilingheight) { // Above-ceiling hack
			tempsec->ceilingheight = s->ceilingheight;
...
static const int checkcoord[12][4] = // killough -- static const
		{{3, 0, 2, 1}, {3, 0, 2, 0}, {3, 1, 2, 0}, {0}, {2, 0, 2, 1}, {0, 0, 0, 0}, {3, 1, 3, 0},
				{0}, {2, 0, 3, 1}, {2, 1, 3, 1}, {2, 1, 3, 0}};
...
static void R_Subsector(int num) {
...
#endif
	{
		...
		frontsector = R_FakeFlat(frontsector, &tempsec, &floorlightlevel, &ceilinglightlevel,
