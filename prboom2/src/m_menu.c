...
enum { newgame = 0, loadgame, savegame, options, readthis, quitdoom, main_end } main_e;
...
menuitem_t EpisodeMenu[] = // added a few free entries for UMAPINFO
		{{1, "M_EPI1", M_Episode, 'k'}, {1, "M_EPI2", M_Episode, 't'},
				...
				{1, "", M_Episode, '0'}, {1, "", M_Episode, '0'}, {1, "", M_Episode, '0'},
				{1, "", M_Episode, '0'}};
...
void M_ChooseSkill(int choice) {
	if (choice == nightmare) { // Ty 03/27/98 - externalized
...
static void SetDefaultSaveName(int slot) {
	...
	if (lumpinfo[maplumpnum].source == source_iwad) {
		...
		snprintf(savegamestrings[itemOn], SAVESTRINGSIZE, "%s (%s)", lumpinfo[maplumpnum].name,
				basename);
		(free)(wadname);
	}
...
// M_QuitDOOM
//
int quitsounds[8] = {sfx_pldeth, sfx_dmpain, sfx_popain, sfx_slop, sfx_telept, sfx_posit1,
		sfx_posit3, sfx_sgtatk};

int quitsounds2[8] = {sfx_vilact, sfx_getpow, sfx_boscub, sfx_slop, sfx_skeswg, sfx_kntdth,
...
menu_t KeybndDef = {generic_setup_end, &SetupDef, Generic_Setup, M_DrawKeybnd, 34,
		5, // skull drawn here
		0};

menu_t WeaponDef = {generic_setup_end, &SetupDef, Generic_Setup, M_DrawWeapons, 34,
		5, // skull drawn here
		0};

menu_t StatusHUDDef = {generic_setup_end, &SetupDef, Generic_Setup, M_DrawStatusHUD, 34,
		5, // skull drawn here
		0};

menu_t AutoMapDef = {generic_setup_end, &SetupDef, Generic_Setup, M_DrawAutoMap, 34,
		5, // skull drawn here
		0};
...
static void M_DrawItem(const setup_menu_t* s) {
	...
	if (flags & S_RESET)
		...
		for (p = t = strdup(s->m_text); (p = strtok(p, "\n"));
				y += 8, p = NULL) { /* killough 10/98: support left-justification: */
...
setup_menu_t keys_settings1[] = // Key Binding screen strings
		{{"MOVEMENT", S_SKIP | S_TITLE, m_null, KB_X, KB_Y},
				...
				{"STRAFE LEFT", S_KEY, m_scrn, KB_X, KB_Y + 6 * 8, {&key_strafeleft}, 0,
						&joybstrafeleft},
				{"STRAFE RIGHT", S_KEY, m_scrn, KB_X, KB_Y + 7 * 8, {&key_straferight}, 0,
						&joybstraferight},
...
setup_menu_t weap_settings1[] = // Weapons Settings screen
		{{"ENABLE RECOIL", S_YESNO, m_null, WP_X, WP_Y + weap_recoil * 8, {"weapon_recoil"}},
				{"ENABLE BOBBING", S_YESNO, m_null, WP_X, WP_Y + weap_bobbing * 8,
						{"player_bobbing"}},
				{"WEAPON ATTACK ALIGNMENT", S_CHOICE, m_null, WP_X,
						WP_Y + weap_attack_alignment * 8, {"weapon_attack_alignment"}, 0, 0, NULL,
						weapon_attack_alignment_strings},

				{"1ST CHOICE WEAPON", S_WEAP, m_null, WP_X, WP_Y + weap_pref1 * 8,
						{"weapon_choice_1"}},
				{"2nd CHOICE WEAPON", S_WEAP, m_null, WP_X, WP_Y + weap_pref2 * 8,
						{"weapon_choice_2"}},
				{"3rd CHOICE WEAPON", S_WEAP, m_null, WP_X, WP_Y + weap_pref3 * 8,
						{"weapon_choice_3"}},
				{"4th CHOICE WEAPON", S_WEAP, m_null, WP_X, WP_Y + weap_pref4 * 8,
						{"weapon_choice_4"}},
				{"5th CHOICE WEAPON", S_WEAP, m_null, WP_X, WP_Y + weap_pref5 * 8,
						{"weapon_choice_5"}},
				{"6th CHOICE WEAPON", S_WEAP, m_null, WP_X, WP_Y + weap_pref6 * 8,
						{"weapon_choice_6"}},
				{"7th CHOICE WEAPON", S_WEAP, m_null, WP_X, WP_Y + weap_pref7 * 8,
						{"weapon_choice_7"}},
				{"8th CHOICE WEAPON", S_WEAP, m_null, WP_X, WP_Y + weap_pref8 * 8,
						{"weapon_choice_8"}},
				{"9th CHOICE WEAPON", S_WEAP, m_null, WP_X, WP_Y + weap_pref9 * 8,
						{"weapon_choice_9"}},
...
setup_menu_t stat_settings1[] = // Status Bar and HUD Settings screen
		{{"STATUS BAR", S_SKIP | S_TITLE, m_null, SB_X, SB_Y + 1 * 8},
				...
				{"SINGLE KEY DISPLAY", S_YESNO, m_null, SB_X, SB_Y + 4 * 8,
				...
				{"ARMOR COLOR DEPENDS ON TYPE", S_YESNO, m_null, SB_X, SB_Y + 10 * 8,
						{"sts_armorcolor_type"}},
...
setup_menu_t stat_settings2[] = {
		...
		{"SHOW TIME/STS ABOVE STATUS BAR", S_YESNO, m_null, ADVHUD_X, SB_Y + 8 * 8,
				{"hudadd_timests"}},

		{"CROSSHAIR SETTINGS", S_SKIP | S_TITLE, m_null, ADVHUD_X, SB_Y + 10 * 8},
		{"ENABLE CROSSHAIR", S_CHOICE, m_null, ADVHUD_X, SB_Y + 11 * 8, {"hudadd_crosshair"}, 0, 0,
				0, crosshair_str},
...
setup_menu_t auto_settings1[] = // 1st AutoMap Settings screen
		{{"Show Kills/Secrts/Items statistics", S_YESNO, m_null, AU_X, AU_Y + 0 * 8,
				 {"map_level_stat"}},
				{"Show coordinates of automap pointer", S_YESNO, m_null, AU_X, AU_Y + 1 * 8,
						{"map_point_coord"}}, // killough 10/98
				{"Show Secrets only after entering", S_YESNO, m_null, AU_X, AU_Y + 2 * 8,
						{"map_secret_after"}},
				{"Update unexplored parts in automap mode", S_YESNO, m_null, AU_X, AU_Y + 3 * 8,
						{"map_always_updates"}},
				{"Grid cell size 8..256, -1 for autosize", S_NUM, m_null, AU_X, AU_Y + 4 * 8,
						{"map_grid_size"}, 0, 0, M_ChangeMapGridSize},
				{"Scroll / Zoom speed  (1..32)", S_NUM, m_null, AU_X, AU_Y + 5 * 8,
						{"map_scroll_speed"}},
				{"Use mouse wheel for zooming", S_YESNO, m_null, AU_X, AU_Y + 6 * 8,
						{"map_wheel_zoom"}},
				{"Apply multisampling", S_YESNO, m_null, AU_X, AU_Y + 7 * 8,
						{"map_use_multisamling"}, 0, 0, M_ChangeMapMultisamling},
#ifdef GL_DOOM
				{"Enable textured display", S_YESNO, m_null, AU_X, AU_Y + 8 * 8, {"map_textured"},
						0, 0, M_ChangeMapTextured},
				{"Things appearance", S_CHOICE, m_null, AU_X, AU_Y + 9 * 8,
						{"map_things_appearance"}, 0, 0, NULL, map_things_appearance_list},
				...
				{"Textured automap in overlay mode", S_NUM, m_null, AU_X, AU_Y + 12 * 8,
						{"map_textured_overlay_trans"}},
				{"Lines in overlay mode", S_NUM, m_null, AU_X, AU_Y + 13 * 8,
						{"map_lines_overlay_trans"}},
#endif
...
setup_menu_t auto_settings2[] = // 2st AutoMap Settings screen
		{{"background", S_COLOR, m_null, AU_X, AU_Y, {"mapcolor_back"}},
				...
				{"line at floor height change", S_COLOR, m_null, AU_X, AU_Y + 3 * 8,
						{"mapcolor_fchg"}},
				{"line at ceiling height change", S_COLOR, m_null, AU_X, AU_Y + 4 * 8,
						{"mapcolor_cchg"}},
				{"line at sector with floor = ceiling", S_COLOR, m_null, AU_X, AU_Y + 5 * 8,
						{"mapcolor_clsd"}},
				...
				{"AUTOMAP LEVEL TITLE COLOR", S_CRITEM, m_null, AU_X, AU_Y + 12 * 8,
						{"hudcolor_titl"}},
				{"AUTOMAP COORDINATES COLOR", S_CRITEM, m_null, AU_X, AU_Y + 13 * 8,
						{"hudcolor_xyco"}},
...
setup_menu_t auto_settings3[] = // 3nd AutoMap Settings screen
		{{"teleporter line", S_COLOR, m_null, AU_X, AU_Y, {"mapcolor_tele"}},
				...
				{"computer map unseen line", S_COLOR, m_null, AU_X, AU_Y + 3 * 8,
						{"mapcolor_unsn"}},
				{"line w/no floor/ceiling changes", S_COLOR, m_null, AU_X, AU_Y + 4 * 8,
						{"mapcolor_flat"}},
				...
				{"your colour in multiplayer", S_COLOR, m_null, AU_X, AU_Y + 10 * 8,
						{"mapcolor_me"}},

				{"friends", S_COLOR, m_null, AU_X, AU_Y + 12 * 8,
						{"mapcolor_frnd"}}, // killough 8/8/98
...
setup_menu_t enem_settings1[] = // Enemy Settings screen
		{
				// killough 7/19/98
				{"Monster Infighting When Provoked", S_YESNO, m_null, E_X,
						E_Y + enem_infighting * 8, {"monster_infighting"}},

				{"Remember Previous Enemy", S_YESNO, m_null, E_X, E_Y + enem_remember * 8,
						{"monsters_remember"}},

				// killough 9/8/98
				{"Monster Backing Out", S_YESNO, m_null, E_X, E_Y + enem_backing * 8,
						{"monster_backing"}},
				...
				// killough 10/98
				{"Affected by Friction", S_YESNO, m_null, E_X, E_Y + enem_friction * 8,
						{"monster_friction"}},

				{"Rescue Dying Friends", S_YESNO, m_null, E_X, E_Y + enem_help_friends * 8,
						{"help_friends"}},

				// killough 7/19/98
				{"Number Of Single-Player Helper Dogs", S_NUM | S_LEVWARN, m_null, E_X,
						E_Y + enem_helpers * 8, {"player_helpers"}},

				// killough 8/8/98
				{"Distance Friends Stay Away", S_NUM, m_null, E_X, E_Y + enem_distfriend * 8,
						{"friend_distance"}},

				{"Allow dogs to jump down", S_YESNO, m_null, E_X, E_Y + enem_dog_jumping * 8,
						{"dog_jumping"}},
...
setup_menu_t gen_settings1[] = { // General Settings screen1
		...
		{"Aspect Ratio", S_CHOICE, m_null, G_X, G_Y + 4 * 8, {"render_aspect"}, 0, 0,
				M_ChangeAspectRatio, render_aspects_list},
		{"Fullscreen Video mode", S_YESNO, m_null, G_X, G_Y + 5 * 8, {"use_fullscreen"}, 0, 0,
				M_ChangeFullScreen},
		{"Status Bar and Menu Appearance", S_CHOICE, m_null, G_X, G_Y + 6 * 8,
				{"render_stretch_hud"}, 0, 0, M_ChangeStretch, render_stretch_list},
		{"Vertical Sync", S_YESNO, m_null, G_X, G_Y + 7 * 8, {"render_vsync"}, 0, 0,
				M_ChangeVideoMode},

		{"Enable Translucency", S_YESNO, m_null, G_X, G_Y + 9 * 8, {"translucency"}, 0, 0, M_Trans},
		{"Translucency filter percentage", S_NUM, m_null, G_X, G_Y + 10 * 8, {"tran_filter_pct"}, 0,
				0, M_Trans},
		...
		{"Number of Sound Channels", S_NUM | S_PRGWARN, m_null, G_X, G_Y + 14 * 8,
				{"snd_channels"}},
		...
		{"Preferred MIDI player", S_CHOICE | S_PRGWARN, m_null, G_X, G_Y + 17 * 8,
				{"snd_midiplayer"}, 0, 0, M_ChangeMIDIPlayer, midiplayers},
...
static const char* gen_compstrings[] = {"17/Default", "0/Doom v1.2", "1/Doom v1.666",
		"2/Doom (2) v1.9", "3/Ultimate v1.9", "4/Final Doom", "5/DosDoom", "6/TASDoom",
		"7/Boom vanilla", "8/Boom v2.01", "9/Boom", "10/LxDoom", "11/MBF", "12/PrB 2.03b",
		"13/PrB 2.1.x", "14/PrB 2.2.x", "15/PrB 2.3.x", "16/PrB 2.4.0", "17/PrB+ Latest", NULL};
...
setup_menu_t gen_settings2[] = { // General Settings screen2
		...
		{"Use In-Game Launcher", S_CHOICE, m_null, G_X, G_Y + 18 * 8, {"launcher_enable"}, 0, 0,
				NULL, launcher_enable_states},
#endif
...
setup_menu_t gen_settings3[] = { // General Settings screen2
		...
		{"Use Extended Format", S_YESNO | S_PRGWARN, m_null, G_X, G_Y + 2 * 8,
				{"demo_extendedformat"}, 0, 0, M_ChangeDemoExtendedFormat},
		...
		{"Smooth Demo Playback Factor", S_NUM, m_null, G_X, G_Y + 5 * 8, {"demo_smoothturnsfactor"},
...
setup_menu_t gen_settings4[] = { // General Settings screen3
		...
		{"Filter for walls", S_CHOICE, m_null, G_X, G_Y + 2 * 8, {"filter_wall"}, 0, 0, NULL,
				renderfilters},
		{"Filter for floors/ceilings", S_CHOICE, m_null, G_X, G_Y + 3 * 8, {"filter_floor"}, 0, 0,
				NULL, renderfilters},
		...
		{"Drawing of sprite edges", S_CHOICE, m_null, G_X, G_Y + 8 * 8, {"sprite_edges"}, 0, 0,
				NULL, edgetypes},
...
setup_menu_t gen_settings5[] = { // General Settings screen3
		...
		{"Screen Multiple Factor (1-None)", S_NUM, m_null, G_X, G_Y + 2 * 8,
				{"render_screen_multiply"}, 0, 0, M_ChangeScreenMultipleFactor},
		...
		{"Thing Sprite Fuzz", S_CHOICE, m_null, G_X, G_Y + 11 * 8, {"gl_thingspritefuzzmode"}, 0, 0,
				0, gl_spritefuzzmodes},
		{"Weapon Sprite Fuzz", S_CHOICE, m_null, G_X, G_Y + 12 * 8, {"gl_weaponspritefuzzmode"}, 0,
				0, 0, gl_spritefuzzmodes},
...
setup_menu_t gen_settings6[] = {{"EMULATION", S_SKIP | S_TITLE, m_null, G_X2, G_Y + 1 * 8},
		...
		{"WARN ON INTERCEPTS OVERFLOW", S_YESNO, m_null, G_X2, G_Y + 6 * 8,
		...
		{"TRY TO EMULATE IT", S_YESNO, m_null, G_X2, G_Y + 13 * 8,
...
setup_menu_t gen_settings7[] = {
		...
		{"LINEDEFS W/O TAGS APPLY LOCALLY", S_YESNO, m_null, G_X2, G_Y + 2 * 8,
				{"comperr_zerotag"}},
		{"USE PASSES THRU ALL SPECIAL LINES", S_YESNO, m_null, G_X2, G_Y + 3 * 8,
				{"comperr_passuse"}},
		{"WALK UNDER SOLID HANGING BODIES", S_YESNO, m_null, G_X2, G_Y + 4 * 8,
				{"comperr_hangsolid"}},
...
setup_menu_t gen_settings8[] = { // General Settings screen4
		...
		{"Patch Filter Mode", S_CHOICE, m_null, G_X, G_Y + 4 * 8, {"gl_patch_filter"}, 0, 0,
				M_ChangeTextureParams, gltexfilters},
		{"Anisotropic filter", S_CHOICE, m_null, G_X, G_Y + 5 * 8,
				{"gl_texture_filter_anisotropic"}, 0, 0, M_ChangeTextureParams,
				gltexfilters_anisotropics},
		...
		{"Enable Internal Hi-Res", S_YESNO, m_null, G_X, G_Y + 9 * 8, {"gl_texture_internal_hires"},
				0, 0, M_ChangeTextureUseHires},
		{"Enable External Hi-Res", S_YESNO, m_null, G_X, G_Y + 10 * 8,
				{"gl_texture_external_hires"}, 0, 0, M_ChangeTextureUseHires},
		...
		{"Enable High Quality Resize", S_YESNO, m_null, G_X, G_Y + 13 * 8, {"gl_texture_hqresize"},
				0, 0, M_ChangeTextureHQResize},
		{"Resize textures", S_CHOICE, m_null, G_X, G_Y + 14 * 8, {"gl_texture_hqresize_textures"},
				0, 0, M_ChangeTextureHQResize, gl_hqresizemodes},
		{"Resize sprites", S_CHOICE, m_null, G_X, G_Y + 15 * 8, {"gl_texture_hqresize_sprites"}, 0,
				0, M_ChangeTextureHQResize, gl_hqresizemodes},
		{"Resize patches", S_CHOICE, m_null, G_X, G_Y + 16 * 8, {"gl_texture_hqresize_patches"}, 0,
				0, M_ChangeTextureHQResize, gl_hqresizemodes},

		{"Allow Detail Textures", S_YESNO, m_null, G_X, G_Y + 18 * 8, {"gl_allow_detail_textures"},
				0, 0, M_ChangeUseDetail},
...
setup_menu_t comp_settings2[] = // Compatibility Settings screen #2
		{{"Tagged doors don't trigger special lighting", S_YESNO, m_null, C_X,
				...
				{"God mode isn't absolute", S_YESNO, m_null, C_X, C_Y + compat_god* COMP_SPC,
						{"comp_god"}},
				...
				{"NEXT ->", S_SKIP | S_NEXT, m_null, KB_NEXT, C_Y + C_NEXTPREV,
						{comp_settings3}}, // e6y
...
setup_menu_t comp_settings3[] = // Compatibility Settings screen #3
		{{"Emulate pre-Ultimate BossDeath behaviour", S_YESNO, m_null, C_X,
				...
				{"No predefined translucency for some things", S_YESNO, m_null, C_X,
						C_Y + compat_translucency* COMP_SPC, {"comp_translucency"}, 0, 0,
...
setup_menu_t mess_settings1[] = // Messages screen
  ...
  {"Message Duration During Play (ms)", S_NUM, m_null, M_X,
   M_Y  + mess_timer*8, {"message_timer"}},
				...
				{"Chat Message Color", S_CRITEM, m_null, M_X, M_Y + mess_color_chat * 8,
						{"hudcolor_chat"}},
  ...
  {"Chat Message Duration (ms)", S_NUM, m_null, M_X,
   M_Y  + mess_chat_timer*8, {"chat_msg_timer"}},
				...
				{"Message Review Color", S_CRITEM, m_null, M_X, M_Y + mess_color_review * 8,
						{"hudcolor_list"}},
  ...
  {"Message Review Duration (ms)", S_NUM, m_null, M_X,
   M_Y  + mess_hud_timer*8, {"hud_msg_timer"}},
				...
				{"Number of Review Message Lines", S_NUM, m_null, M_X, M_Y + mess_lines * 8,
						{"hud_msg_lines"}},
  ...
  {"Message Listing Scrolls Upwards",  S_YESNO,  m_null,  M_X,
   M_Y + mess_scrollup*8, {"hud_msg_scrollup"}},
				...
				{"Message Background", S_YESNO, m_null, M_X, M_Y + mess_background * 8,
						{"hud_list_bgon"}},
...
setup_menu_t helpstrings[] = // HELP screen strings
		{{"SCREEN", S_SKIP | S_TITLE, m_null, KT_X1, KT_Y1},
				...
				{"TURN RIGHT", S_SKIP | S_KEY, m_null, KT_X3, KT_Y3 + 4 * 8, {&key_right},
						&mousebturnright},
				{"RUN", S_SKIP | S_KEY, m_null, KT_X3, KT_Y3 + 5 * 8, {&key_speed}, &mousebspeed,
						&joybspeed},
