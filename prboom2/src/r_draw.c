...
typedef enum { COL_NONE, COL_OPAQUE, COL_TRANS, COL_FLEXTRANS, COL_FUZZ, COL_FLEXADD } columntype_e;
...
static const int fuzzoffset_org[FUZZTABLE] = {FUZZOFF, -FUZZOFF, FUZZOFF, -FUZZOFF, FUZZOFF,
...
static R_DrawColumn_f
		drawcolumnfuncs[VID_MODEMAX][RDRAW_FILTER_MAXFILTERS][RDRAW_FILTER_MAXFILTERS]
...
R_DrawColumn_f R_GetDrawColumnFunc(enum column_pipeline_e type, enum draw_filter_type_e filter,
...
void R_FillBackScreen(void) {
	...
	if (ratio_multiplier != ratio_scale || wide_offsety) {
		...
		if (only_stbar && ST_SCALED_OFFSETX > 0) {
			...
			V_FillFlat(grnrock.lumpnum, 1, 0, stbar_top, ST_SCALED_OFFSETX, ST_SCALED_HEIGHT,
					VPT_NONE);
	...
	V_FillPatch(brdr_t.lumpnum, 1, viewwindowx, viewwindowy - 8, scaledviewwidth, brdr_t.height,
			VPT_NONE);
	...
	V_DrawNumPatch(viewwindowx + scaledviewwidth, viewwindowy - 8, 1, brdr_tr.lumpnum, CR_DEFAULT,
			VPT_NONE);
