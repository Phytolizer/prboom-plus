	...
	while (leftout != leftend) {
		...
		for (chan = 0; chan < numChannels; chan++) {
			...
			if (ci->data) {
				...
				// quality
				if (ci->bits == 16) {
					s = (short)(ci->data[0] | (ci->data[1] << 8)) *
									(255 - (ci->stepremainder >> 8)) +
...
void I_ResampleStream(void* dest, unsigned nsamp, void (*proc)(void* dest, unsigned nsamp),
		unsigned sratein, unsigned srateout) { // assumes 16 bit signed interleaved stereo
	...
	for (i = 0; i < nsamp; i++) {
		*sout++ =
				((unsigned)sin[j + 0] * (0x10000 - remainder) + (unsigned)sin[j + 2] * remainder) >>
				16;
		*sout++ =
				((unsigned)sin[j + 1] * (0x10000 - remainder) + (unsigned)sin[j + 3] * remainder) >>
				16;
		remainder += step;
		j += remainder >> 16 << 1;
		remainder &= 0xffff;
...
#endif
	lprintf(LO_INFO,
			"I_InitMusic: Was compiled without SDL_Mixer support.  You should enable experimental "
	        "music.\n");
}
