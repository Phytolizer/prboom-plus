...
enum { NO_ZDOOM_NODES, ZDOOM_XNOD_NODES, ZDOOM_ZNOD_NODES };
...
static int P_CheckForZDoomUncompressedNodes(int lumpnum, int gl_lumpnum) {
	...
	if (result) {
		...
		if (result) {
			lprintf(LO_INFO,
					"P_CheckForZDoomUncompressedNodes: compressed ZDoom nodes are detected\n");
...
static void P_LoadSegs(int lump) {
	...
	for (i = 0; i < numsegs; i++) {
		...
		if ((unsigned)linedef >= (unsigned)numlines) {
			I_Error("P_LoadSegs: seg %d references a non-existent linedef %d", i,
					(unsigned)linedef);
		}
...
static void P_LoadSegs_V4(int lump) {
	...
	for (i = 0; i < numsegs; i++) {
		...
		// e6y: check for wrong indexes
		if ((unsigned)linedef >= (unsigned)numlines) {
			I_Error("P_LoadSegs_V4: seg %d references a non-existent linedef %d", i,
					(unsigned)linedef);
		}
		...
		// http://www.doomworld.com/idgames/index.php?id=12647
		if (v1 >= numvertexes || v2 >= numvertexes) {
			char str[200] = "P_LoadSegs_V4: compatibility loss - seg %d references a non-existent "
			                "vertex %d\n";
...
static void P_LoadZSegs(const byte* data) {
	...
	for (i = 0; i < numsegs; i++) {
		...
		if ((unsigned int)linedef >= (unsigned int)numlines) {
			I_Error("P_LoadZSegs: seg %d references a non-existent linedef %d", i,
					(unsigned)linedef);
		}
...
static void P_LoadBlockMap(int lump) {
	...
	if (blockmapxneg != -257 || blockmapyneg != -257) {
		lprintf(LO_WARN,
				...
				"in the \"Compatibility with common mapping errors\" menu in order to activate a "
		        "fix. "
