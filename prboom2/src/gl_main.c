...
void gld_InitTextureParams(void) {
	...
	tex_filter_t params[filter_count] = {
			...
			{true, GL_NEAREST, GL_NEAREST_MIPMAP_NEAREST, "GL_NEAREST",
...
void gld_MapDrawSubsectors(
	...
	if (numsubsectors > visible_subsectors_size) {
		...
		visible_subsectors = realloc(
				visible_subsectors, visible_subsectors_size * sizeof(visible_subsectors[0]));
	}
		...
		if (gltexture) {
			...
			for (loopnum = 0; loopnum < subsectorloops[ssidx].loopcount; loopnum++) {
				...
				for (vertexnum = currentloop->vertexindex;
						...
						vertexnum++) {
					glTexCoord2f(flats_vbo[vertexnum].u + floor_uoffs,
							flats_vbo[vertexnum].v + floor_voffs);
...
void gld_DrawWeapon(int weaponlump, vissprite_t* vis, int lightlevel) {
	...
	y1 = viewwindowy + centery -
			(int)(((float)vis->texturemid / (float)FRACUNIT) * pspriteyscale_f);
...
unsigned char* gld_ReadScreen(void) { // NSM convert to static
...
void gld_EndDrawScene(void) {
	...
	if (!viewangleoffset && !viewpitchoffset) { // don't draw on side views
			...
			case GLDIT_MWALL:
				...
				if (anim) {
					if (itemtype == GLDIT_WALL || itemtype == GLDIT_MWALL) {
						wall->alpha = 1.0f -
								((float)tic_vars.frac +
										((leveltime - 1) % anim->speed) * 65536.0f) /
										(65536.0f * anim->speed);
...
static void gld_DrawWall(GLWall* wall) {
	...
	has_detail = scene_has_details && gl_arb_multitexture && (wall->flag < GLDWF_SKY) &&
			...
			gld_IsDetailVisible(xCamera, yCamera, wall->glseg->x1, wall->glseg->z1, wall->glseg->x2,
					wall->glseg->z2);
...
#define CALC_Y_VALUES(w, lineheight, floor_height, ceiling_height) \
	...
	lineheight = ((float)fabs( \
			((ceiling_height) / (float)FRACUNIT) - ((floor_height) / (float)FRACUNIT)))

#define OU(w, seg) \
	(((float)((seg)->sidedef->textureoffset) / (float)FRACUNIT) / \
			(float)(w).gltexture->buffer_width)
#define OV(w, seg) \
	(((float)((seg)->sidedef->rowoffset) / (float)FRACUNIT) / (float)(w).gltexture->buffer_height)
#define OV_PEG(w, seg, v_offset) \
	(OV((w), (seg)) - (((float)(v_offset) / (float)FRACUNIT) / (float)(w).gltexture->buffer_height))
...
void gld_AddWall(seg_t* seg) {
	...
	if (!seg->backsector) /* onesided */
	{
		...
		if (temptex && frontsector->ceilingheight > frontsector->floorheight) {
			...
			CALC_TEX_VALUES_MIDDLE1S(wall, seg, backseg, (LINE->flags & ML_DONTPEGBOTTOM) > 0,
	...
	} else /* twosided */
	{
		...
		if (frontsector->ceilingpic == skyflatnum) // || backsector->ceilingpic==skyflatnum)
		{
			...
			if (
				...
				if (bs->ceilingpic == skyflatnum && fs->ceilingpic != skyflatnum &&
					...
					if (((backsector->ceilingpic != skyflatnum && toptexture != NO_TEXTURE) &&
				...
				if (!temptex && gl_use_stencil && backsector &&
					...
					if (wall.ybottom >= zCamera) {
						...
						temptex = gld_RegisterFlat(
								flattranslation[seg->backsector->ceilingpic], true);
						if (temptex) {
							wall.gltexture = temptex;
					...
					CALC_TEX_VALUES_TOP(wall, seg, backseg,
							(LINE->flags & (/*e6y ML_DONTPEGBOTTOM | */ ML_DONTPEGTOP)) == 0,
							linelength, lineheight);
					gld_AddDrawWallItem(GLDIT_WALL, &wall);
				}
			}
		...
		if (temptex && seg->sidedef->midtexture != NO_TEXTURE &&
			...
			wall.vt = (float)((-top + ceiling_height) >> FRACBITS) /
	...
	bottomtexture:
		...
		if (floor_height < ceiling_height) {
			...
			if (!temptex && gl_use_stencil && backsector &&
					!(seg->linedef->r_flags & RF_ISOLATED) &&
					/*frontsector->floorpic != skyflatnum && */ backsector->floorpic !=
							skyflatnum &&
					!(backsector->flags & NULL_SECTOR)) {
				wall.ytop = ((float)(ceiling_height) / (float)MAP_SCALE) + SMALLDELTA;
				wall.ybottom = ((float)(floor_height) / (float)MAP_SCALE) - SMALLDELTA;
		...
		if (!no_overlapped_sprites) {
			...
			while (i < count) {
				...
				if (sprite1->xy == sprite2->xy) {
					...
					i++;
					while (i < count &&
							gld_drawinfo.items[itemtype][i].item.sprite->xy == sprite1->xy) {
